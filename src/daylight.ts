...
const dayLight = () => {
  ...
  */
  ...
  const skycolors = chroma.scale(["#aaaaaa", "#ffeeb1", "#aaaaaa"]);
  const groundcolors = chroma.scale(["#ffcf73", "#080820", "#ffcf73"]);
  ...
  const setTime = ({ hour = 1, minutes = 0 }) => {
    ...
    hemisphereLight.color.setHex(ThreeHex(hemiColors[0].toString()));
    hemisphereLight.groundColor.setHex(Number(hemiColors[1]));
    hemisphereLight.intensity = Number(hemiColors[2]);
